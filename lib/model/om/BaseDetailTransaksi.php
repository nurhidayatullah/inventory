...
abstract class BaseDetailTransaksi extends BaseObject implements Persistent
{
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 4; // 4 = DetailTransaksiPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = DetailTransaksiQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(DetailTransaksiPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(DetailTransaksiPeer::ID_TRANSAKSI)) {
            $modifiedColumns[':p' . $index++]  = '`id_transaksi`';
        }
        if ($this->isColumnModified(DetailTransaksiPeer::ID_BARANG)) {
            $modifiedColumns[':p' . $index++]  = '`id_barang`';
        }
        if ($this->isColumnModified(DetailTransaksiPeer::JUMLAH)) {
            $modifiedColumns[':p' . $index++]  = '`jumlah`';
        }

        $sql = sprintf(
            'INSERT INTO `detail_transaksi` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`id`':
                        ...
                        break;
                    case '`id_transaksi`':
                        $stmt->bindValue($identifier, $this->id_transaksi, PDO::PARAM_INT);
                        break;
                    case '`id_barang`':
    ...
    {
        if (isset($alreadyDumpedObjects['DetailTransaksi'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['DetailTransaksi'][$this->getPrimaryKey()] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aTransaksi) {
                $result['Transaksi'] = $this->aTransaksi->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
    ...
    {
        $copyObj->setIdTransaksi($this->getIdTransaksi());
     ...
     * @throws PropelException
     */
    public function getTransaksi(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aTransaksi === null && ($this->id_transaksi !== null) && $doQuery) {
            $this->aTransaksi = TransaksiQuery::create()->findPk($this->id_transaksi, $con);
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->aTransaksi instanceof Persistent) {
              $this->aTransaksi->clearAllReferences($deep);
            }
    ...
    {
        return (string) $this->exportTo(DetailTransaksiPeer::DEFAULT_STRING_FORMAT);
    }
    ...
    public function __call($name, $params)
    {
        ...
        // symfony_behaviors behavior
        if ($callable = sfMixer::getCallable('BaseDetailTransaksi:' . $name))
        {
