    ...
    if ($colorize)
    {
      ...
      $message = preg_replace_callback(
        ...
        function ($matches) use ($self) {
          return $self->colorizer->colorize($matches[1], 'ERROR');
        },
        $message
      );
      $message = preg_replace_callback(
        '/(?:^|\.)(ok *\d*)\b/',
        function ($matches) use ($self) {
          return $self->colorizer->colorize($matches[1], 'INFO');
        },
        $message
      );
      $message = preg_replace_callback(
        '/"(.+?)"/',
        function ($matches) use ($self) {
          return $self->colorizer->colorize($matches[1], 'PARAMETER');
        },
        $message
      );
      $message = preg_replace_callback(
        '/(\->|\:\:)?([a-zA-Z0-9_]+?)\(\)/',
        function ($matches) use ($self) {
          return $self->colorizer->colorize($matches[1].$matches[2].'()', 'PARAMETER');
        },
        $message
      );
    }
